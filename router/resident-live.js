...
function createGroup(name){const section=document.createElement('section');section.className='device-group';section.innerHTML='<div class="device-heading"><h2></h2><div class="controls"><button type="button">start audio</button><label>base octave <select><option>1</option><option>2</option><option selected>3</option><option>4</option><option>5</option></select></label><label>pitch span <input class="span" type="range" min="1" max="6" step="1" value="3"><span class="readout">3</span> oct</label><label>volume <input class="volume" type="range" min="0" max="2.5" step="0.01" value="0.8"><span class="readout volume-value">0.80</span></label><span class="audio-status">audio off</span></div></div><table><thead><tr><th>STREAM</th><th>STATUS</th><th>COVERAGE</th><th>VOICES</th></tr></thead><tbody></tbody></table>';
const heading=section.querySelector('h2');heading.textContent=name;const button=section.querySelector('button');const baseOctave=section.querySelector('select');const span=section.querySelector('.span');const spanValue=span.nextElementSibling;const volume=section.querySelector('.volume');const volumeValue=section.querySelector('.volume-value');const audioStatus=section.querySelector('.audio-status');const body=section.querySelector('tbody');
const group={name,section,body,rows:new Map(),enabled:false,bus:null,button,baseOctave,span,spanValue,volume,volumeValue,audioStatus};groups.set(name,group);groupsRoot.appendChild(section);
...
baseOctave.addEventListener('change',syncAudio);span.addEventListener('input',()=>{spanValue.textContent=span.value;syncAudio()});volume.addEventListener('input',()=>{volumeValue.textContent=Number(volume.value).toFixed(2);syncAudio()});return group}
function mappedAudioFrequency(residentHz,group){const minResidentHz=0.05;const maxResidentHz=1;const clamped=Math.max(minResidentHz,Math.min(maxResidentHz,residentHz));const position=Math.log2(clamped/minResidentHz)/Math.log2(maxResidentHz/minResidentHz);const lowMidi=(Number(group.baseOctave.value)+1)*12;const midi=lowMidi+position*(Number(group.span.value)*12);return 440*Math.pow(2,(midi-69)/12)}
