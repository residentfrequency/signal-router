...
</style></head><body><h1>RESIDENT FREQUENCY VOICES</h1><div class="status" id="status">connecting…</div><div class="midi-panel"><button id="midi-enable" type="button">enable MIDI</button><label>output <select id="midi-output" disabled><option value="">none</option></select></label><button id="midi-panic" type="button" disabled>panic</button><span class="midi-status" id="midi-status">MIDI unavailable until enabled</span></div><main id="groups"></main>
<script>
...
let midiAccess=null;let midiOutput=null;let midiEnabled=false;const midiVoices=new Map();const MIDI_HYSTERESIS=0.6;
const midiEnable=document.getElementById('midi-enable');const midiOutputSelect=document.getElementById('midi-output');const midiPanic=document.getElementById('midi-panic');const midiStatus=document.getElementById('midi-status');
...
function groupMidiChannel(name){if(name==='indoor-sky')return 0;if(name==='electric-sky')return 1;const names=[...groups.keys()].filter(value=>value!=='indoor-sky'&&value!=='electric-sky').sort();return Math.min(15,2+Math.max(0,names.indexOf(name)))}
...
function sendNoteOff(channel,note){if(midiOutput)midiOutput.send([0x80|channel,note,0])}
...
function syncMidi(forceRetune=false){if(!midiEnabled||!midiOutput)return;const activeKeys=new Set();for(const group of groups.values()){const channel=groupMidiChannel(group.name);for(const voice of activeVoicesFor(group.name)){const key=voiceKey(voice.streamId,voice.id);activeKeys.add(key);const mapped=mappedMidiValues(voice.frequencyHz,group);const existing=midiVoices.get(key);const note=forceRetune?mapped.quantizedMidi:hystereticNote(mapped.continuousMidi,existing&&existing.note);const velocity=midiVelocity(voice.confidence);if(!existing){sendNoteOn(channel,note,velocity);midiVoices.set(key,{channel,note,velocity})}else if(existing.note!==note||existing.channel!==channel){sendNoteOff(existing.channel,existing.note);sendNoteOn(channel,note,velocity);midiVoices.set(key,{channel,note,velocity})}}}for(const [key,state] of midiVoices){if(activeKeys.has(key))continue;sendNoteOff(state.channel,state.note);midiVoices.delete(key)}updateMidiStatus()}
