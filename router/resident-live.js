...
pitchMode.addEventListener('change',syncAudio);baseOctave.addEventListener('change',syncAudio);span.addEventListener('input',()=>{spanValue.textContent=span.value;syncAudio()});volume.addEventListener('input',()=>{volumeValue.textContent=Number(volume.value).toFixed(2);syncAudio()});return group}
