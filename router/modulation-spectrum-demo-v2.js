...
let spectrumFps=30,lastSpectrumAt=0,source='waiting for router sample batches',messages=0,types=new Map(),lastStreams='';
...
function noteType(m){const t=m&&m.type||'untyped';types.set(t,(types.get(t)||0)+1)}
...
function describeStreams(streams){return streams.map(s=>{const a=Array.isArray(s.samples)?s.samples:[],name=String(s.device||s.name||s.param||'stream');if(a.length<2)return name+':'+a.length;const dt=(Number(a.at(-1)?.[1])-Number(a[0]?.[1]))/1e6,rate=dt>0?(a.length-1)/dt:0;return name+':'+a.length+'@'+Math.round(rate)+'Hz'}).join(' | ')}
function chooseAudio(streams){const candidates=streams.map(s=>{const samples=Array.isArray(s.samples)?s.samples.filter(x=>Array.isArray(x)&&Number.isFinite(Number(x[1]))&&Number.isFinite(Number(x[2]))):[];if(samples.length<32)return null;const duration=(Number(samples.at(-1)[1])-Number(samples[0][1]))/1e6;const rate=duration>0?(samples.length-1)/duration:0;const name=String(s.device||s.name||s.param||'stream');const audioHint=/audio|mic|inmp|pcm|wave/i.test(name)?1:0;return{s,samples,rate,name,score:audioHint*1e9+rate}}).filter(Boolean);candidates.sort((a,b)=>b.score-a.score);return candidates[0]||null}
function spectrumFromSamples(c){const x=c.samples.map(v=>Number(v[2]));const n=x.length,sr=c.rate;if(!(sr>1000)||n<32)return null;let mean=0;for(const v of x)mean+=v;mean/=n;const mags=new Float32Array(B);for(let b=0;b<B;b++){const hz=FMIN*Math.pow(FMAX/FMIN,(b+.5)/B);if(hz>=sr/2)continue;let re=0,im=0,ws=0;for(let j=0;j<n;j++){const w=.5-.5*Math.cos(2*Math.PI*j/Math.max(1,n-1)),v=(x[j]-mean)*w,p=2*Math.PI*hz*j/sr;re+=v*Math.cos(p);im-=v*Math.sin(p);ws+=w}mags[b]=Math.log1p(Math.hypot(re,im)*8/Math.max(1,ws))}return{mags,label:c.name,sr}}
function ingest(spec){const now=performance.now();if(lastSpectrumAt){const f=1000/Math.max(1,now-lastSpectrumAt);spectrumFps=.9*spectrumFps+.1*f}lastSpectrumAt=now;source=spec.label+' · '+Math.round(spec.sr)+' Hz samples';for(let b=0;b<B;b++){const v=spec.mags[b],p=pos[b];hist[b][p]=v;pos[b]=(p+1)%H;filled[b]=Math.min(H,filled[b]+1);display[b][0]=Math.max(v,display[b][0]*DECAY)}computeMod()}
...
function draw(){const w=canvas.width,h=canvas.height,d=devicePixelRatio||1,lw=62*d,pw=w-lw,cw=pw/(M+1),ch=h/B;ctx.fillStyle='#030408';ctx.fillRect(0,0,w,h);for(let b=0;b<B;b++){const y=h-(b+1)*ch;for(let c=0;c<=M;c++){ctx.fillStyle=color(display[b][c]);ctx.fillRect(lw+c*cw,y,Math.ceil(cw+.5),Math.ceil(ch+.5))}}ctx.fillStyle='rgba(255,255,255,.22)';ctx.fillRect(lw+cw,0,1,h);ctx.font=10*d+'px ui-monospace';ctx.textAlign='right';ctx.textBaseline='middle';ctx.fillStyle='#8a94a3';[20,50,100,200,500,1000,2000,4000,8000].forEach(hz=>{const y=h-Math.log(hz/FMIN)/Math.log(FMAX/FMIN)*h;ctx.fillText(hz>=1000?(hz/1000)+'k':hz,lw-8,y)});const observed=[...types.entries()].sort((a,b)=>b[1]-a[1]).slice(0,3).map(x=>x[0]).join(', ');status.textContent=source+' · '+spectrumFps.toFixed(1)+' spectrum fps · '+messages+' msgs'+(observed?' · '+observed:'')+(lastStreams?' · '+lastStreams:'');requestAnimationFrame(draw)}
function connect(){const ws=new WebSocket((location.protocol==='https:'?'wss://':'ws://')+location.host);ws.onopen=()=>status.textContent='connected · waiting for router data';ws.onmessage=e=>{messages++;try{const m=JSON.parse(e.data);noteType(m);const streams=streamsFrom(m);if(streams.length)lastStreams=describeStreams(streams);const c=chooseAudio(streams);if(c){const s=spectrumFromSamples(c);if(s)ingest(s)}}catch(err){status.textContent='message parse error: '+err.message}};ws.onclose=()=>{status.textContent='disconnected · retrying';setTimeout(connect,1500)};ws.onerror=()=>ws.close()}
connect();draw();
})();
</script></body></html>`;

function summarizeSampleBatch(message) {
  ...
  return streams.map(stream => {
    ...
    const name = stream.device || stream.name || stream.param || 'stream';
    if (samples.length < 2) return { name, samples: samples.length, rateHz: null };
    const firstUs = Number(samples[0]?.[1]);
    const lastUs = Number(samples.at(-1)?.[1]);
    const durationSeconds = (lastUs - firstUs) / 1e6;
    const rateHz = durationSeconds > 0 ? (samples.length - 1) / durationSeconds : null;
    return { name, samples: samples.length, rateHz: Number.isFinite(rateHz) ? Math.round(rateHz) : null, first: samples[0], last: samples.at(-1) };
  });
}
...
function startDemo(){
 ...
 const clients=new WebSocketServer({server});let router,retry,count=0,loggedBatch=false;
 const connect=()=>{router=new WebSocket(ROUTER_URL,{rejectUnauthorized:false});router.on('open',()=>console.log(`Modulation demo connected to ${ROUTER_URL}`));router.on('message',raw=>{count++;const text=raw.toString();if(count<=5)console.log(`Router message ${count}: ${text.slice(0,240)}`);if(!loggedBatch){try{const message=JSON.parse(text);if(message.type==='sample_batch'){loggedBatch=true;console.log('First sample_batch streams:',JSON.stringify(summarizeSampleBatch(message),null,2))}}catch{}}for(const c of clients.clients)if(c.readyState===WebSocket.OPEN)c.send(text)});router.on('close',()=>{console.log('Modulation demo router connection closed; retrying');retry=setTimeout(connect,2000)});router.on('error',err=>console.error('Modulation demo router error:',err.message))};connect();
