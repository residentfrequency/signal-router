...
function decodePcm(buffer){const data=new DataView(buffer);if(data.byteLength<32||String.fromCharCode(data.getUint8(0),data.getUint8(1),data.getUint8(2),data.getUint8(3))!=='ESAU')return null;const bits=data.getUint8(6),count=data.getUint16(24,true),header=data.getUint16(26,true),payload=bits===16?count*2:bits===4?Math.ceil((count-1)/2):-1;if(payload<0||header+payload!==data.byteLength||bits===4&&header<36)return null;const samples=new Float32Array(count);if(bits===16){for(let i=0;i<count;i++)samples[i]=data.getInt16(header+i*2,true)/32768;return samples}let predictor=data.getInt16(32,true),index=Math.max(0,Math.min(88,data.getUint8(34)));samples[0]=predictor/32768;for(let i=1;i<count;i++){const packed=data.getUint8(header+Math.floor((i-1)/2)),code=(i&1)?packed&15:packed>>4,step=IMA_STEP[index];let delta=step>>3;if(code&4)delta+=step;if(code&2)delta+=step>>1;if(code&1)delta+=step>>2;predictor+=code&8?-delta:delta;predictor=Math.max(-32768,Math.min(32767,predictor));index=Math.max(0,Math.min(88,index+IMA_INDEX[code&7]));samples[i]=predictor/32768}return samples}
